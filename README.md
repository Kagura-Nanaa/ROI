...
所有代码均由Gemini 3 Pro生成，包括此README.md（不包括这一行），这个app只是我自己使用，所以bug和新功能就完全随缘了，能用就行

Surprise 是一款专为漫画爱好者和本地视频收藏者设计的 Android 应用程序。它旨在解决“不知道下一本看什么”的选择困难症，通过智能随机算法，从你庞大的本地收藏库中“抽取”一份惊喜。

该项目完全基于 Kotlin 开发，采用了最新的 Material You (Material 3) 设计语言，界面现代、简洁且美观。
...
一键随机：只需点击按钮，App 会自动扫描指定根目录下的所有子文件夹，随机选中一个，并打开其中的文件。

多格式支持：智能识别并打开图片文件（jpg, png, webp, bmp）和视频文件（mp4, mkv, avi, mov）。

无缝对接：自动调用你最喜欢的外部阅读器（如 Perfect Viewer）或播放器（如 MX Player）打开文件，并支持翻页浏览（通过路径解析技术实现）。
...
多书架支持：你可以添加多个不同的根目录（例如“已完结漫画”、“连载中”、“视频收藏”），它们会以 Chip（标签按钮） 的形式显示在主页。

自定义别名：系统路径太长难记？你可以为每个文件夹设置一个简短的别名（如“我的最爱”）。

灵活编辑：长按标签即可进行管理操作，支持重命名、更改底层路径（保持别名不变）或移除。
...
全局控制：开启悬浮窗服务后，会在屏幕边缘显示一个半透明的悬浮球。

随时随地：无论你在使用什么 App，点击悬浮球即可立即触发随机打开功能，无需切回主程序。

智能贴边：悬浮球支持拖拽移动，松手后自动吸附到屏幕左右边缘，不遮挡视线。

高度定制：支持在设置中调节悬浮窗的大小，甚至可以设置 App 启动时自动开启悬浮窗。

🚀 高级特性 (Advanced Features)

1. 智能冷却模式 (Cooldown Mode)

防止重复：担心短时间内随机到同一本漫画？开启冷却模式后，App 会记录已打开文件夹的时间戳。

自定义时长：你可以设置冷却时间（例如 60 分钟），在此期间内，已看过的文件夹将不会再次被随机选中。

2. 文件夹黑名单 (Folder Exclusion)

精准过滤：长按任意路径标签，进入“排除子文件夹”菜单。

保持清爽：勾选你不希望看到的子文件夹（如 Cover、Thumbnails、广告 等），随机算法会自动跳过它们。

3. 高度可配置

指定应用：在设置中，你可以自由填写图片查看器和视频播放器的包名，让 App 能够调用任何你喜欢的第三方软件。

🎨 技术亮点 (Technical Highlights)

Material Design 3：全面采用 Google 最新的设计规范，支持动态取色（Dynamic Colors），UI 颜色随手机壁纸自动变化。

Android 14+ 适配：完美支持最新的 Android 系统特性，包括针对 Android 11+ 的分区存储（Scoped Storage）适配和全文件访问权限（MANAGE_EXTERNAL_STORAGE）处理。

Uri to Path 转换：解决了 Android 系统文件选择器（SAF）返回的虚拟 URI 无法被部分老牌阅读器（如 Perfect Viewer）识别的问题，自动转换为真实文件路径。

纯 Kotlin 编写：代码简洁高效，无原生库（No Native Libraries），体积轻量，兼容性极佳。

English

Surprise App (Random Reader)
All code in this project was generated by Gemini 3 Pro, including this README.md (excluding this specific line). This app is intended solely for my personal use; therefore, bug fixes and new features are strictly "as fate permits"—as long as it works, I'm happy.
...
漫画囤积症患者：收藏了成千上万本漫画，每次打开阅读器都在列表里滑半天不知道看哪本。

本地视频浏览：拥有大量本地短视频或番剧，想要随机播放一集打发时间。

寻找灵感：设计师或画师从本地素材库中随机抽取图片寻找灵感。
